...
An Advanced Algorithms exam project (single author, Luca Simonetti) investigating whether 1D CNNs have anything algorithmic in common with exact string matching, in two directions:
...
The deliverable is the paper (`paper.md` → `paper.tex` → `paper.pdf`). The central empirical result: a single-filter CNN (and even a K=4 ReLU multi-filter net) converges to a PSSM whose per-window logit obeys an exact additive identity `logit(W) = π − Σ pen[i, W[i]]`, where `pen[i,c] = w[i,P_i] − w[i,c] ≥ 0`. This `pen` table is a real-valued, position-aware generalisation of the Boyer–Moore bad-character rule — not a failure function (CNNs are stateless; `sp` requires sequential state, cf. Merrill 2019).
